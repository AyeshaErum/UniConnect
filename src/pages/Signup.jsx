...
export default function Signup() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      toast.success('Account created! Welcome to UniConnect.')
      navigate('/')
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-hero-pattern px-4 py-8">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="font-display text-3xl font-bold text-white">Join UniConnect</h1>
          <p className="text-navy-300 mt-1">Connect, Learn & Grow with your campus</p>
        </div>
        ...
        <div className="bg-navy-900/90 backdrop-blur border border-navy-700 rounded-2xl p-6 shadow-card space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <Input
              ...
              placeholder="Alex Chen" error={errors.name} required
            />
            <Input
              label="Email" type="email" icon={Mail} value={form.email}
              onChange={e => set('email', e.target.value)}
              placeholder="you@university.edu" error={errors.email} required
            />
            <Input
              label="Password" type="password" icon={Lock} value={form.password}
              onChange={e => set('password', e.target.value)}
              placeholder="Min 6 characters" error={errors.password} required
            />
            <Input
              label="Confirm Password" type="password" icon={Lock} value={form.confirm}
